...
class ShoppingAnalytics:
    ...
    def get_total_spending(self, time_value: int, time_unit: TimeFrame) -> float:
        ...
        query = """
            ...
            WHERE date_time >= %s;
        """
        return self.db.fetch_one(query, (start_date,))['total'] or 0.0
    ...
    def get_product_spending(self, product_name: str, time_value: int, time_unit: TimeFrame) -> ShoppingStats:
        ...
        start_date = self._parse_timeframe(time_value, time_unit)
        product_name = str(unicodedata.normalize('NFKD', product_name.upper()).encode('ASCII', 'ignore').decode('ASCII'))
        query = """
            SELECT 
                SUM(bi.amount * bi.price) / 10000 as total_spent,
                SUM(bi.amount) as total_items,
    ...
    def get_shop_spending(self, shop_id: int, time_value: int, time_unit: TimeFrame) -> ShoppingStats:
        ...
        return ShoppingStats(
            total_spent=result['total_spent'],
            item_count=result['visit_count'],
            avg_price=result['total_spent'] / result['visit_count'],
            first_purchase=result['first_visit'],
            last_purchase=result['last_visit']
        )
    ...
    def get_most_visited_shop(self, time_value: int, time_unit: TimeFrame) -> Tuple[int, str, int]:
        ...
        query = """
            ...
            GROUP BY bi.shop_id, s.name
            ORDER BY visit_count DESC
            LIMIT 1;
        """
        result = self.db.fetch_one(query, (start_date,))
        return (result['shop_id'], result['name'], result['visit_count']) if result else None
    ...
    def get_price_changes(self, product_name: str) -> List[Dict]:
        """Track price changes for a specific product over time"""
        query = """
            SELECT 
                date_time,
                price,
        ...
        """
        return self.db.fetch_all(query, (product_name,))
